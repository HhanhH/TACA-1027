...
public class CookieUtil {
	...
	public static void saveCookie(String username, String password,
		...
		// MD5加密用户详细信息
		String cookieValueWithMd5 = getMD5(username + ":" + password + ":"
				+ validTime + ":" + webKey);
		// 将要被保存的完整的Cookie值
		String cookieValue = username + ":" + validTime + ":"
				+ cookieValueWithMd5;
		// 再一次对Cookie的值进行BASE64编码
		// String cookieValueBase64 = new
		// String(Base64Util.encode(cookieValue.getBytes()));
		// 开始保存Cookie
		Cookie cookie = new Cookie(cookieDomainName, cookieValueWithMd5);
	...
	public static void readCookieAndLogon(HttpServletRequest request,
		...
		// (Base64Util.decode(cookieValue),"utf-8");
		...
		// 根据用户名到数据库中检查用户是否存在
		UserInfo user = userInfoAction.getUserInfoByUM(username);
